# App-adapted version of plotCellTypePCA function
# This version includes all functionality from the original package function

plotCellTypePCA_app <- function(query_data,
                                reference_data,
                                query_cell_type_col,
                                ref_cell_type_col,
                                cell_types = NULL,
    ...
    # Basic input validation (simplified version of argumentCheck for app)
    ...
    # Validate facet arguments (equivalent to match.arg)
    valid_lower <- c("scatter", "contour", "ellipse", "blank")
    valid_diagonal <- c("ridge", "density", "boxplot")
    valid_upper <- c("blank", "scatter", "contour", "ellipse")
    
    if (!lower_facet %in% valid_lower) {
        stop(paste("lower_facet must be one of:", paste(valid_lower, collapse = ", ")))
    }
    if (!diagonal_facet %in% valid_diagonal) {
        stop(paste("diagonal_facet must be one of:", paste(valid_diagonal, collapse = ", ")))
    }
    if (!upper_facet %in% valid_upper) {
        stop(paste("upper_facet must be one of:", paste(valid_upper, collapse = ", ")))
    }
    ...
    # Get the projected PCA data using the support function
    ...
    if (nrow(pca_output) == 0) {
        stop("No data available for the selected cell types")
    }
    ...
    # Generate colors for cell types and datasets using support function
